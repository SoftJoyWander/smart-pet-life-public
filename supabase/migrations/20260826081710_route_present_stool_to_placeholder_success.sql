-- Prototype routing for the stool-presence model. The candidate is an
-- uncalibrated model output, not a diagnosis or a measured probability.
...
begin
  ...
  machine_candidate := case
    when stool_raw_response ->> 'candidate_code' in ('present', 'absent', 'uncertain')
      then stool_raw_response ->> 'candidate_code'
    else 'uncertain'
  end;
  review_needed := machine_candidate <> 'present';
  routed_result := case when machine_candidate = 'present' then 'present' else 'uncertain' end;

  recorded_response := coalesce(stool_raw_response, '{}'::jsonb) || jsonb_build_object(
    'database_routing', jsonb_build_object(
      'candidate_code', machine_candidate,
      'review_required', review_needed,
      'result_code', routed_result,
      'policy', 'prototype_present_candidate_to_placeholder_v1'
    )
  );
